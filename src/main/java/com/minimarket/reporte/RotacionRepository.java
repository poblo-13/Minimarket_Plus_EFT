...
public interface RotacionRepository extends JpaRepository<DetalleVenta, Long> {
    @Query("select d.producto.id, d.producto.nombre, sum(d.cantidad), sum(d.precio * d.cantidad) " +
           "from DetalleVenta d where d.venta.fecha >= :desde and d.venta.fecha < :hasta " +
           "and (:sucursalId is null or d.venta.sucursal.id = :sucursalId) " +
           "group by d.producto.id, d.producto.nombre order by sum(d.cantidad) desc, d.producto.id asc")
