...
public class AdministracionSucursalController {
    ...
    private EntityModel<StockSucursalResponse> model(com.minimarket.sucursal.StockSucursal s) {
        Long sucursalId = s.getSucursal().getId();
        Long productoId = s.getProducto().getId();
        return EntityModel.of(new StockSucursalResponse(sucursalId, productoId, s.getDisponible(), s.getStockMinimo()),
                linkTo(methodOn(SucursalController.class).consultarDisponibilidad(sucursalId, productoId)).withSelfRel(),
                linkTo(methodOn(AdministracionSucursalController.class).obtener(sucursalId)).withRel("sucursal"));
    }
}
